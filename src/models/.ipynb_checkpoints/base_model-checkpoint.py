...
class BasePortfolioOptimizer(nn.Module, ABC):
    ...
    def _estimate_parameters(self, asset_data, common_data):
        """모델 파라미터 추정"""
        # 베타 및 기대 수익률 예측
    ...
    def forward(self, asset_data, common_data, future_returns, future_common_data, 
                allow_short_selling: bool = True, **kwargs):
        """순전파"""
        # 1. 데이터 전처리
        ...
        # 2. 모델 파라미터 추정
        betas, expected_returns, beta_ols, return_ols = self._estimate_parameters(asset_data, common_data)
        
        # 3. 공분산 행렬 계산
        cov_matrix = self._calculate_covariance(asset_data, common_data, betas)
